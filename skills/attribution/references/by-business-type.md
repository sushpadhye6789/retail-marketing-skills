...
Attribution defaults differ sharply by business model. The same "which channel drives revenue?" question wants a different source of truth, model, and paradigm depending on how long your cycle is, how many people are involved, and where your budget goes. Three playbooks: B2B/Wholesale, Ecommerce/DTC, and the two blended cases below. Match the user's product to one (or blend, for a business running both a self-serve Business-segment motion and a sales-assisted Commercial-segment motion — see `marketing-strategy` Section 16 — or a dealer-mediated trade channel alongside a growing direct channel, see Dealer/Trade + Direct Omnichannel below).
...
1. **Source of truth = the CRM**, not any analytics tool. Revenue is real in the CRM (closed-won, annual contract value); everything else explains where those deals came from. Pipeline and revenue attribution live in the wholesale/trade CRM itself — attribution feeds it the "source" dimension (no dedicated pipeline-management skill covers this; see `marketing-strategy/references/b2b-wholesale-guide.md`'s noted gap).
...
## Dealer/Trade + Direct Omnichannel

A different blended shape from the one above: a manufacturer selling through an independent dealer network (a genuine trade purchase — long consideration, sales-assisted, matching the B2B/Wholesale pattern) that also runs a growing direct-to-consumer ecommerce channel (short cycle, self-serve, matching the DTC pattern) at the same time. This isn't two B2B segments of the same motion — it's a retail/trade channel and a consumer channel running in parallel for the same products, and it's common enough in dealer-mediated retail that it's worth naming directly rather than re-deriving each time:

- Use the **B2B/Wholesale approach for the dealer/trade channel** — self-reported attribution anchored at the point of the dealer-assisted sale is the strongest signal here, since that buying decision genuinely involves research and consideration a dealer's sales staff walk the buyer through.
- Use the **DTC approach for the direct channel** — standard MTA/last-touch, post-purchase survey, the usual ecommerce toolkit.
- **Avoid double-counting a customer who researches via a dealer relationship but converts online, or vice versa.** This is the specific failure mode this blend introduces that the Business+Commercial blend above doesn't: a buyer can walk out of a dealer showroom having been sold on the product, then complete the purchase on the direct site later (or the reverse — research online, buy through a dealer for local service/installation). Where identity can be linked (a loyalty account, a warranty registration, a dealer-referral code), alias the two touchpoints into one journey rather than crediting both channels independently for the same sale. Where it can't be linked, say so explicitly rather than silently summing both channels' attributed revenue — the same "don't sum platform-reported conversions" discipline the DTC trap above names, applied across channels instead of across platforms.
- **This is also the fork for measuring co-op/MDF funding's incrementality** (the specific case `supplier-funded-marketing` defers to this skill): compare sell-through for dealers who used MDF-funded campaigns against a matched set of dealers who didn't — a holdout comparison, not just raw fund-utilization numbers.
- **Post-transaction, opt-in dealer-selection commission is a third, thinner attribution case, not a full dealer-assisted sale.** Where a direct-channel sale lets the customer optionally select a dealer for commission after checkout (see `marketing-strategy/references/distribution-model-guide.md`'s Attribution and measurement section for the full mechanic), the commission-attributed dealer revenue **understates** true dealer influence, because selection is easy to skip even by a customer who genuinely researched at a dealer first. Track the select-through rate as its own diagnostic (by category/region/dealer density), and report commission-attributed revenue and estimated true dealer-influenced revenue as two separate numbers rather than letting the cleaner, smaller one stand in for the messier, larger one.
