...
- **Design the fund structure deliberately** — accrual-based (tied to dealer purchase volume, scales naturally with the relationship), flat allocation (simpler to administer, less tied to actual dealer performance), or matching funds (you match dealer spend up to a cap, which self-selects for dealers actually willing to invest). Each has different incentive effects; pick based on what behavior you actually want to encourage. **A franchise national ad fund is a distinct fourth structure** — a mandatory, non-optional percentage-of-revenue contribution from every franchisee, pooled and spent centrally (often on national/regional brand campaigns) rather than reimbursed against individual local claims; see `marketing-strategy/references/distribution-model-guide.md`'s franchise note for how this changes the enforcement and compliance expectations relative to a standard dealer MDF program.
