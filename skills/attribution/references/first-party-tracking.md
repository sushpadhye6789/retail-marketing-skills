...
- **Hand the record off to the CRM/pipeline process.** Once the source/confidence/basis + paid/organic live on the account, how pipeline and lifecycle *use* them (routing, lead scoring, nurture branching, revenue attribution reporting) is downstream of this runbook — `prospecting` covers the qualification/scoring half; ongoing CRM-pipeline routing doesn't have a dedicated skill (see `marketing-strategy/references/b2b-wholesale-guide.md`). This runbook's job is to get a trustworthy, labeled source onto the record.
